    ...
    <div className="min-h-screen bg-[#f8f6f1] px-5 py-10 text-[#173b3b] lg:px-8">
      <div className="mx-auto max-w-2xl">
        <p className="text-xs font-bold uppercase tracking-wide text-[#173b3b]/45">
          <Link href="/" className="hover:underline">{dict.account.breadcrumbHome}</Link> / {dict.account.myAccount}
        </p>
        <h1 className="mt-2 font-serif text-4xl font-bold tracking-tight">{dict.account.myAccount}</h1>

        <div className="mt-6 rounded-2xl border border-[#173b3b]/10 bg-white p-6">
          <p className="font-bold">{current.name}</p>
          <p className="mt-1 text-sm text-[#173b3b]/55">{current.role === 'customer' ? dict.account.customerAccount : dict.account.staffAccount.replace('{role}', current.role)}</p>
          {isStaff(current.role) && (
            <Link href="/admin" className="mt-3 inline-block rounded-full bg-[#173b3b] px-4 py-2 text-xs font-bold text-white">{dict.account.openAdminConsole}</Link>
          )}
        </div>
