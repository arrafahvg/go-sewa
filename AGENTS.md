...
- **Money**: integer minor units (100 units = Rp 1) in `*Cents` columns. Format with
  `formatMoney()` from `lib/utils/money.ts`; convert staff-entered Rupiah with
  `rupiahToCents()` / `centsToRupiah()` from the same module. Everything is IDR/Rupiah.
...
npm test             # Vitest suite (unit tests; integration tests need TEST_DATABASE_URL)
...
| 6 | Admin CRM + check-out/check-in/inspection | ✅ CRM (customers + leads), walk-in form, booking detail (device assignment, check-out/in, inspection, deposit & payment recording) done. Rental extensions (§29) live on the booking detail page: server-side availability re-check over the extra window (conflicts refused, never silently approved), endsAt + total updated via the product's daily rule, audit-logged with in-panel history. Per-product `deposit_required` flag editable in inventory AND enforced at checkout (forces ID document + deposit hold). Customer detail page `/admin/customers/[id]` (§31B) live: clickable profiles with lifetime stats, ID-document viewer (signed URLs), invoice/agreement deep links, WhatsApp quick-contact and audited inline editing of contact info. Customer selection in walk-in/manual invoice/agreement forms uses a dedicated `CustomerPicker` (Existing/New modes, auto-fills phone+email, reuses the exact customer id) |
| 7–9 | Invoices, agreements, deposits, maintenance ops | ✅ done — invoices & agreements generation; deposit/payment actions + UI; maintenance + damage admin UI with per-report damage-charge entry, charge notes and optional deposit forfeiture (writes `damage_charges` + a `forfeited` deposit transaction); verified-ID enforcement at check-out for deposit-required products. Document templates editable at `/admin/templates` (§21B structured fields + live preview + draft re-render prompt); documents support real PDF download and revocable public share links (`/d/[token]`) for WhatsApp/email. Manual (booking-less) invoice creation is live at `/admin/invoices` (§35): staff pick/reuse a customer and enter free-form line items stored in `invoice_line_items`; rendered on the admin invoice page and shared `/d` view. Manual (booking-less) rental agreements are live at `/admin/agreements` (§21/§35): staff pick/reuse a customer and enter equipment/terms lines stored in `agreement_line_items` and merged from the active template; list page added and Agreements added to the admin sidebar. Manual payments instead of a gateway: staff-configured bank accounts, QRIS image and instructions (settings §73) render on invoices & share views incl. print/PDF; manual invoices can override the accounts/QRIS/instructions per invoice (snapshotted on the invoice row) |
| 10 | CMS admin | ✅ done — `/admin/settings` company profile (name, managed logo upload, address, email, phone, WhatsApp, Instagram, Maps, footer tagline) applied to storefront shell, plus `/admin/content` content CMS (homepage hero incl. admin-managed hero image with bundled placeholder fallback `public/hero-placeholder.svg`, FAQ, testimonials add/edit/toggle/delete, bilingual (ID/EN) homepage hero copy with per-field EN fallback, home-page SEO title/description/noindex fed into `generateMetadata`) rendered on the storefront home page (§42). Admin layout made dynamic/fluid: data pages fill available width next to the sidebar; only document/form pages keep narrow columns. Structured content (JSONB sections + separate faq/testimonials tables); admin-managed browser favicon (`favicon_url` setting, bundled `/favicon.svg` default, upload/reset at /admin/settings) |
| 11 | Tracking architecture | ✅ live — provider abstraction (`lib/services/tracking/provider.ts`, explicit integration point), `device_tracking_configurations` / `device_tracking_events` tables (migration `0008`), per-device enrollment + honest "Tracking integration not configured" states in the devices table; locations only ever come from a real connected provider (§80) |
| 12 | SEO / perf / a11y / tests / i18n | ✅ done — Vitest test suite (`npm test`: money/dates/tracking-provider unit coverage plus DB-backed integration tests for overdue automation and booking conflict prevention, gated on `TEST_DATABASE_URL`); sitemap.xml + robots.txt live (product pages respect `noindex`); ID/EN i18n (§9) live for the entire customer-facing surface with an ID/EN switcher; the admin console remains English (staff tool). Per-product OG/Twitter metadata done; a11y pass done (skip link, focus-visible ring, aria-live regions, expanded/pressed/controls semantics on all interactive controls) |
