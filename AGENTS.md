...
| 12 | SEO / perf / a11y / tests / i18n | 🟡 partial — Vitest test suite (`npm test`: money/dates/tracking-provider unit coverage plus DB-backed integration tests for overdue automation and booking conflict prevention, gated on `TEST_DATABASE_URL`); sitemap.xml + robots.txt live (product pages respect `noindex`); ID/EN i18n (§9) live for the entire customer-facing surface with an ID/EN switcher; the admin console remains English (staff tool). Per-product OG/Twitter metadata done; a full a11y pass still open |
