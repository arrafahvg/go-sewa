...
| 12 | SEO / perf / a11y / tests / i18n | 🟡 partial — Vitest test suite (`npm test`: money/dates/tracking-provider unit coverage plus a DB-backed overdue automation integration test gated on `TEST_DATABASE_URL`); ID/EN i18n (§9) live for the entire customer-facing surface — storefront shell, home, rent list, product detail, checkout flow and account pages incl. localized booking statuses — with an ID/EN switcher; the admin console remains English (staff tool) |
