...
| 12 | SEO / perf / a11y / tests / i18n | 🟡 partial — Vitest test suite (`npm test`, money/dates/tracking-provider coverage); ID/EN i18n (§9) live for the storefront shell, home, rent list and product detail with an ID/EN switcher (cookie-based, falls back to `default_language` setting); checkout flow strings, account pages and the admin console still pending extraction |
